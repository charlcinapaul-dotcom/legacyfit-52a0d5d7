...
- Run a Python script to detect and replace all white/near-white pixels (RGB > 240) with the parchment color (#D8C29A) that matches the map's natural edge color
...
- In `src/pages/Landing.tsx`, add a parchment-toned background color (`bg-[#D8C29A]`) to the image container `div` so that if the image doesn't fully cover on any breakpoint, the exposed area blends seamlessly with the map edges instead of showing gray

### Files changed

- `src/pages/Landing.tsx` — add `bg-[#D8C29A]` class to the image wrapper div (line 55)
