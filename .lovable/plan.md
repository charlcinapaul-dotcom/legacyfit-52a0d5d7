...
- Run a Python script to detect and replace all white/near-white pixels (RGB > 240) with the parchment color (#F0E2C6) that matches the map's natural edge color
...
- In `src/pages/Landing.tsx`, add a parchment-toned background color (`bg-[#F0E2C6]`) to the image container `div` so that if the image doesn't fully cover on any breakpoint, the exposed area blends seamlessly with the map edges instead of showing gray

### Files changed

- `src/pages/Landing.tsx` — add `bg-[#F0E2C6]` class to the image wrapper div (line 55)
