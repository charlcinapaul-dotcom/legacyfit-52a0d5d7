...
const compactDisclaimer =
  "LegacyFit is an educational platform. All historical figures and references are included for educational purposes only. We are not affiliated with any individual or estate referenced.";
  ...
  if (variant === "inline") {
    return (
      <div className={cn("text-xs text-muted-foreground text-center", className)}>
        ...
        {showLivingPersonNote && <p className="mt-1 italic">{livingPersonNote}</p>}
      </div>
    );
  }
      ...
      <div className={cn("p-4 rounded-lg bg-secondary/50 border border-border", className)}>
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm text-muted-foreground">
            ...
            {showLivingPersonNote && <p className="mt-2 font-medium text-foreground/80">{livingPersonNote}</p>}
          </div>
        </div>
      </div>
    );
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="space-y-3">
          ...
          {showLivingPersonNote && (
            <p className="text-sm font-medium text-primary border-l-2 border-primary pl-3 mt-4">{livingPersonNote}</p>
          )}
        </div>
      </div>
    </div>
