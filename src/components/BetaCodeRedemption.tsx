...
const getAccentClasses = (color: BetaCodeRedemptionProps["editionColor"]) => {
  switch (color) {
    case "burgundy":
      return {
        text: "text-[#7A1E2C]",
...
export function BetaCodeRedemption({ challengeId, editionColor = "gold", onRedeemed }: BetaCodeRedemptionProps) {
  ...
  const handleRedeem = async () => {
    if (!code.trim()) {
      toast.error("Please enter a beta access code.");
      return;
    }
    ...
    try {
      ...
      toast.success(data?.message || "Beta code redeemed! You're enrolled.");
      setRedeemed(true);
    ...
    return (
      <div className={cn("flex items-center justify-center gap-2 py-3 px-4 rounded-lg", accent.bg)}>
        ...
        <span className={cn("text-sm font-medium", accent.text)}>
          Beta access granted — you're enrolled!
        </span>
      </div>
    );
  }
  ...
  return (
    <div className={cn("rounded-lg border p-4 space-y-3", accent.bg, accent.border)}>
      <div className="flex items-center gap-2">
        <ShieldCheck className={cn("w-4 h-4", accent.text)} />
        <span className="text-sm font-medium text-foreground">Enter your beta access code</span>
      </div>
      <div className="flex gap-2">
        <Input
          placeholder="e.g. LEGACY2025"
          value={code}
          onChange={(e) => setCode(e.target.value.toUpperCase())}
          disabled={loading}
          className={cn("font-mono text-sm uppercase", accent.border)}
